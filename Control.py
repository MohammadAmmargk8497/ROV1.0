...
class Control(Controller):
    ...
    def map_values(self, value):
        """
        Simple mapping function (Linear mapping for forward controls)
        """
    ...
    def map_values_depth(self, value):
        """
        Simple Linear Mapping for Depth ( for manual depth control when no PID is used)
        """
        if value < -1 or value > 1:
            return None
    ...
    def sig(self, value):
        """
        Custom mapping function for smooth and precise control over turning.
        """
        if value < -1 or value > 1:
            return None
        elif value == 0:
